...
**Файл даних:** `data/network_equipment.json`

**Обґрунтування вибору** Як студент спеціальності «Комп’ютерна інженерія», я вже кілька років вивчаю мережеві технології, тому дана тема є для мене зрозумілою та близькою. Обрана область дозволяє зручно структурувати характеристики комутаторів, маршрутизаторів та точок доступу, що ідеально підходить для реалізації динамічного каталогу з пошуком, фільтрацією та сортуванням. 
...
| `manufacturer` | рядок | виробник обладанння | фільтрація за брендом |
| `type` | рядок | тип пристрою | категоризація та фільтр |
| `ports` | число | кількість портів | фільтр та технічна характеристика |
| `bandwidth` | число | пропускна здатність | фільтрація за швидкістю, текст на картці |
...
**Кількість комітів на момент здачі:** 7
